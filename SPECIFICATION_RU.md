...
`aws-spot-region-selector` — консольный инструмент, который выбирает предпочтительный AWS Region для краткосрочной рабочей нагрузки на EC2 Spot Instances. Выбор является компромиссом между сетевой задержкой от машины, где запущен инструмент, стоимостью Spot, устойчивостью цены и вероятностью получения требуемой ёмкости.
...
Если `--config` не указан, команда автоматически читает `config.yaml` из текущего рабочего каталога. При запуске без аргументов и без файла по умолчанию приложение показывает справку и успешно завершается. Явно заданный отсутствующий файл считается ошибкой конфигурации.
...
В таблице отображается только суффикс имени AZ (`a` для `eu-west-1a`). JSON сохраняет полное account-relative имя AZ. Все денежные значения в таблице содержат ровно четыре знака после запятой; JSON сохраняет полную decimal-точность.
...
Для каждого исключённого региона сохраняются machine-readable reason code и текстовое пояснение. Для `above_limit` пояснение содержит измеренный RTT, заданный предел, единицу измерения и выбранную метрику. Поле `diagnostics` зарезервировано для безопасной machine-readable диагностики и остаётся пустым объектом, если данных нет. Секреты, access keys и session tokens никогда не выводятся.
...
1. Валидный `config.yaml` из раздела 7 автоматически читается без `--config`, и все параметры обычного запуска задаются через него.
