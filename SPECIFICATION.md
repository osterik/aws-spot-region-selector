...
- **Region prefix** — первые две латинские буквы Region ID, например `eu` для `eu-west-1`, `ap` для `ap-south-1`. В конфигурации это поле называется `excluded_region_prefixes`.
...
Термин «zone» не используется для двухбуквенных префиксов, поскольку в AWS он обычно означает Availability Zone.
...
2. Исключать регионы по точному Region ID, glob-маске и двухбуквенному префиксу.
...
### 5.2 Исключение географических групп

Пользователь может заранее исключить все регионы с префиксами, например `us` и `ap`, из-за требований к данным, комплаенсу или стоимости передачи данных.

### 5.3 Исключение отдельных регионов

Пользователь может исключить, например, `eu-west-1`, оставив остальные регионы `eu` доступными.
...
| `regions.excluded_region_prefixes` | Двухбуквенные группы (`eu`, `us`, `ap`); это не Availability Zones. |
...
### 7.2 Правила исключения регионов

Фильтрация выполняется до latency- и pricing-запросов.

- `excluded_region_prefixes` содержит только нормализованные двухбуквенные значения по регулярному выражению `^[a-z]{2}$`.
- Префикс Region определяется первыми двумя символами Region ID после приведения к нижнему регистру.
...
- Исключения имеют безусловный приоритет над всеми остальными настройками и CLI-фильтрами включения.
...
```yaml
regions:
  excluded_region_prefixes: [us, ap]
  excluded_regions: [eu-west-1, eu-central-*]
```

исключает `us-east-1`, `us-west-2`, `ap-south-1` и другие регионы с этими префиксами, только `eu-west-1` из группы `eu`, а также любой обнаруженный регион, совпавший с `eu-central-*`.
...
- Region исключён точным правилом, glob-маской или по префиксу;
...
- `eu` исключает `eu-west-1` и `eu-central-1`, но не `us-east-1`.
...
- Префиксы и Region ID нормализуются по регистру.
- Некорректные префиксы (`e`, `europe`, `1a`, пустая строка) отклоняются.
...
2. Исключения по двухбуквенным префиксам, точным Region ID и glob-маскам работают согласно разделу 7.2.
...
2. Discovery регионов и исключения.
...
- Добавлять ли allowlist регионов; при появлении она не должна отменять exclusions.
